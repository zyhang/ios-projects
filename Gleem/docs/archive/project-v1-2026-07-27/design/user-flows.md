...
A2 Allow Safari Protection（唯一操作指引页）
  1. Open the Settings app
     Tap Apps → Safari → Extensions
  2. Enable Stillwall
     Tap Stillwall, turn the switch on, then choose Allow for All Websites
  3. Return to Stillwall
     We'll check automatically when you come back
  ↓ Open Settings
用户完成系统设置
  ↓ 返回 App，自动检测
  ├── 未启用 → 仍显示 A2（无稍后完成）
  └── 已启用 → 直接进入 Overview / Home（状态 = All Quiet）
        ↓
...
- 强调 Safari 负责执行规则。
- 说明基础扩展不能查看或上报浏览内容。
- A2 用系统路径写清操作（Apps → Safari → Extensions；开启开关 + All Websites）；实现可深链，文案不暴露 Shortcuts。
...
- 未开启时**仍显示 A2 同一操作指引**（不另做「仍未开启」状态页）。
- 若系统状态无法准确判断，继续 A2 指引，不伪装为 `All Quiet`。
