...
> **已归档（2026-07-27，project-v1）**：本文件为归档前 `docs/README.md` 快照，不再作为现行导航。见 [归档说明](README.md)。重构请写到 `docs/README.md`。

> `Gleem` 仅为内部项目代号，App 对外产品名采用 `Stillwall`。在招募公开测试用户前，仍需完成 App Store 重名、域名和目标市场商标核查；如核查不通过则重新决策。
...
- [设计工作区说明](design/README.md)：当前流程与设计稿重构状态、索引与约束。
- [设计原则与视觉规范](design/design-system.md)：品牌人格、视觉语言、组件、动效和无障碍要求（活文档）。
- App 完整流程 / 用户流程 / Lunacy 流程图：**重构中**；上一轮已封存至 [archive/design-v2-2026-07-27](archive/design-v2-2026-07-27/)。
