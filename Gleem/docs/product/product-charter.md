...
3. **Home 状态区（只读）：**  
   - `protected`：授权完成 **且** 至少一个会参与拦截的类别开关为 On。  
   - `off`：授权完成 **且** 所有类别开关均为 Off（无有效拦截）。  
...
| 区域 | 要求 |
|------|------|
| 上：状态区 | 保护中 / 已关闭（**只读**，无全局 Switch）；一句解释 |
...
| 2026-07-29 | **移除主 App 全局保护开关**（D-315）：Home 仅类别开关；状态区只读；恢复路径 = 关类别 / 扩展 Pause |
...
- 主 App：类别开关 + 只读状态 + 订阅 + 引导门禁（**无**全局总开关）。  
