...
| 可恢复 | 误杀时：关全局开关，或在 **Safari 扩展**中放行站点 / 调整；Help 指向该路径 |
...
| 能力 | 层级 | 默认 | 说明 |
|------|------|------|------|
| 全局保护开关 | 免费 | 开 | App 内一键开/关全部拦截；**非**定时 Pause |
| Ads | 免费 | 开 | Safari 常见广告 |
| Privacy | 免费 | 开 | 广告追踪器 |
| Annoyances | 免费 | 开 | Cookie / 隐私提示等烦人项 |
| Regional Ad Blocking | 免费 | 开 | 按**系统语言**全自动，无手动选区负担 |
...
- 免费用户完整可用：全局开关 + Ads / Privacy / Annoyances / Regional + Safari 扩展内站点控制。
...
- **默认：** 关闭。  
- **风险：** 可能造成个别站点异常；恢复路径见 §5.9 / §5.10（关全局、关 Strict，或在 Safari 扩展中放行站点）。  
...
- **做什么：** App Home **一个**主开关：开启 / 关闭 Stillwall 在 Safari 中的拦截（影响已启用的能力类别）。  
- **形态：** 简单 On/Off；**不做**「暂停 15 分钟 / 1 小时 / 直到手动恢复」等定时 Pause UI。  
- **关闭时：** 状态区显示未保护 / Off；不得显示「保护中」。  
- **与类别开关关系：** 全局关 → 全部不拦截；全局开 → 按 Ads/Privacy/… 各自开关生效。  
- **扩展：** popup **不提供**全局开关；全局 Off 时顶栏显示 Off in app 并可引导回主 App。
...
2. **全局保护开关关闭：** 所有拦截不生效；状态为 Off / 未保护。  
3. **全局开 + 类别开关：** 仅已开启的类别参与拦截。  
...
| 主 App 职责 | 引导授权、全局开关、能力类别、订阅、帮助/反馈/关于 |
| VPN | **不使用** |
...
| **主 App 定时 Pause（15m / 1h / Until resume）** | 仅保留全局 On/Off；需要临时放行时用扩展或关全局 |
...
1. 新用户能以较少步骤完成 Safari 授权并进入 Home，状态为保护中。  
2. 默认开启全局保护 + Ads/Privacy/Annoyances/Regional 后，预定站点上广告与追踪明显改善。  
3. 用户能用 **一个全局开关** 关闭/打开保护；站点例外路径在 Help 中可找到（Safari 扩展），**不**要求主 App 内名单。
...
- 主 App：全局开关 + 类别开关 + 订阅 + 引导门禁。  
- 扩展 popup：仅 3 项；无全局开关、无类别列表、无 IAP。  
- 不做：主 App 定时 Pause、主 App Allowed Sites、paywall bypass。
